...
import { connectDB, isMongoDB } from '../config/db';
...
async function main() {
  ...
  if (isMongoDB()) {
    ...
    async function getOrCreateMongoUser(name: string, email: string, role: 'ATTENDEE' | 'ORGANIZER' | 'ADMIN') {
      let u = await MongoUser.findOne({ email });
      if (!u) {
        u = await MongoUser.create({ name, email, password: hashedPassword, role });
      }
      return u;
    }
    ...
    for (const item of sampleEventsData) {
      ...
      const ev = await MongoEvent.create({
        ...eventFields,
        status: 'PUBLISHED',
