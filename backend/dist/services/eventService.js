...
class EventService {
    static async createEvent(organizerId, input) {
        ...
        if ((0, db_1.isMongoDB)()) {
            ...
            if (ticketTypes && ticketTypes.length > 0) {
                createdTicketTypes = await TicketType_1.TicketType.insertMany(ticketTypes.map((t) => ({
            ...
            const organizer = await User_1.User.findById(organizerId).select('id name email');
    ...
    static async getEvents(params) {
        ...
        try {
            if ((0, db_1.isMongoDB)()) {
                ...
                if (totalItems > 0) {
                    const events = await Promise.all(mongoEvents.map(async (ev) => {
                        const tiers = await TicketType_1.TicketType.find({ eventId: ev._id });
                        const evObj = ev.toJSON();
    ...
    static async getEventById(eventId) {
        try {
            if ((0, db_1.isMongoDB)()) {
                const ev = await Event_1.Event.findById(eventId).populate('organizerId', 'name email');
                if (ev) {
                    const tiers = await TicketType_1.TicketType.find({ eventId });
                    const evObj = ev.toJSON();
                    const organizer = evObj.organizerId;
                    delete evObj.organizerId;
                    return {
        ...
        if (fallback)
            return fallback;
        throw new appError_1.AppError('Event or movie record not found', 404);
    }
    static async updateEvent(eventId, userId, userRole, input) {
        if ((0, db_1.isMongoDB)()) {
            const event = await Event_1.Event.findById(eventId);
            if (!event)
                throw new appError_1.AppError('Event not found', 404);
            if (event.organizerId.toString() !== userId && userRole !== types_1.Role.ADMIN) {
                throw new appError_1.AppError('You do not have permission to update this event', 403);
            }
            if (input.title)
            ...
            if (!event)
                throw new appError_1.AppError('Event not found', 404);
            if (event.organizerId !== userId && userRole !== types_1.Role.ADMIN) {
                throw new appError_1.AppError('You do not have permission to update this event', 403);
            }
    ...
    static async deleteEvent(eventId, userId, userRole) {
        if ((0, db_1.isMongoDB)()) {
            const event = await Event_1.Event.findById(eventId);
            if (!event)
                throw new appError_1.AppError('Event not found', 404);
            if (event.organizerId.toString() !== userId && userRole !== types_1.Role.ADMIN) {
                throw new appError_1.AppError('You do not have permission to delete this event', 403);
            }
            await TicketType_1.TicketType.deleteMany({ eventId });
    ...
    static async addTicketType(eventId, userId, userRole, ticketData) {
        if ((0, db_1.isMongoDB)()) {
            const event = await Event_1.Event.findById(eventId);
            if (!event)
                throw new appError_1.AppError('Event not found', 404);
            if (event.organizerId.toString() !== userId && userRole !== types_1.Role.ADMIN) {
                throw new appError_1.AppError('You do not have permission to modify tickets for this event', 403);
            }
            const tier = await TicketType_1.TicketType.create({
                eventId,
            ...
            return await prisma_1.prisma.ticketType.create({
                data: {
                    eventId,
                    name: ticketData.name,
                    price: ticketData.price,
    ...
    static async getTicketTypesByEvent(eventId) {
        if ((0, db_1.isMongoDB)()) {
            const tiers = await TicketType_1.TicketType.find({ eventId });
            ...
            if (!event)
                throw new appError_1.AppError('Event not found', 404);
            return await prisma_1.prisma.ticketType.findMany({ where: { eventId } });
        }
    }
}
